...
const deleteKey = async (keyId: string) => {
  ...
  u.userData.publicKeys = u.userData.publicKeys.filter(
    (key) => key.id !== keyId,
  );
};
...
<template>
  <div class="flex flex-col justify-center items-center mt-16 gap-4">
    <FormButtonInvisible @click="generateKey" class="w-80 lg:w-1/3"
      >Générer une nouvelle clé</FormButtonInvisible
    >

    <FormCard title="Clés publiques sur le serveur" class="lg:w-1/3">
      <div v-for="key in user.publicKeys" class="flex justify-between">
        <div>
          <p>Clé {{ key.id }} du {{ key.createdAt }}</p>
          <NuxtLink
            :to="
            ...
            Télécharger la clé publique</NuxtLink
          >
        </div>

        <FormButtonInvisible class="w-52" @click="deleteKey(key.id)"
          ><svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
          ...
          <span class="mt-0.5 ml-1.5">Supprimer</span>
        </FormButtonInvisible>
      </div>
    </FormCard>
  </div>
